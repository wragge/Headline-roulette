...
$(function(){
    ...
    function process_results(results, type) {
        if (type == 'total') {
            ...
            var query = trove_api_url + "&q=canberra+date:[" + current_year + " TO " + current_year + "]&s=" + number + "&n=1&l-category=Article&encoding=json&key=" + trove_api_key;
    ...
    function fail() {
        $("#guesses").text(0);
        $("#status").html('Oooops! Follow the link to read the article, or hit reload to try again.');
        $("#pub_details").show();
    }
    function reset() {
        $("#year").val("");
