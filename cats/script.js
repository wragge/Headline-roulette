...
$(function(){
    ...
    function get_random_article() {
        $("#headline").text('Choosing a random cat...');
        $("#article").showLoading();
        current_year = get_random_year();
        var query = trove_api_url + "&q=fulltext:cat+date:[" + current_year + " TO " + current_year + "]&n=0&l-category=Article&encoding=json&key=" + trove_api_key;
    ...
    function process_results(results, type) {
        if (type == 'total') {
            ...
            var query = trove_api_url + "&q=fulltext:cat+date:[" + current_year + " TO " + current_year + "]&s=" + number + "&n=1&l-category=Article&encoding=json&key=" + trove_api_key;
    ...
    function correct(guesses) {
        $("#status").html("<b>Meeoow! That's it!</b><br>" + success_messages[guesses]).removeClass("alert-error").addClass("alert-success");
        $("#pub_details").show();
    }
    ...
    function fail() {
        $("#guesses").text(0);
        $("#status").html('Phssst! <img src="http://www.memehumor.com/wp-content/uploads/2010/10/fail_trollcat.jpg"> Follow the link to read the article, or hit reload to try again.');
        $("#pub_details").show();
    }
    function reset() {
        $("#year").val("");
